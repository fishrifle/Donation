(function() {
  // PassItOn Donation Widget Embed Script - LOCAL TESTING VERSION
  ...
  function createDonationWidget(config) {
    ...
    // LOCAL TESTING - Use localhost for development
    const widgetDomain = 'http://localhost:3000'; // Change port if needed
    ...
    // Handle messages from iframe (for success/completion and height adjustment)
    window.addEventListener('message', function(event) {
      ...
      } else if (event.data.type === 'PASSITON_RESIZE') {
        // Dynamically adjust iframe height based on content
        const newHeight = Math.max(400, Math.min(event.data.height + 20, window.innerHeight * 0.8));
